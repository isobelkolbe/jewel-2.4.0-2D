...
// Minor modification by Isobel Kolbe, 2023 (marked `IK`)
#include <iostream>
#include <sstream>
#include <fstream>
#include <cmath>
...
// IK: Attempt to write out tau x and y
void FluidcellStatistic::outputTandUasTauvsXvsY() {
    cout << "output 2D contour plot for temperature and local velocity "
         << "as function of x and y in tau files ... " << endl;
    double grid_tau0, grid_tauf;
    double grid_x0;
    double grid_y0; //IK
    ...
    if (hydro_type == 0) {
        grid_tau0 = hydroinfo_ptr->getHydrogridTau0();      
        grid_tauf = hydroinfo_ptr->getHydrogridTaumax();
        grid_x0 = - hydroinfo_ptr->getHydrogridXmax();      // IK: son't use this
    } else {
        grid_tau0 = hydroinfo_MUSIC_ptr->get_hydro_tau0();
        grid_tauf = hydroinfo_MUSIC_ptr->get_hydro_tau_max();
        grid_x0 = (- hydroinfo_MUSIC_ptr->get_hydro_x_max()
    ...
    for (int itime = 0; itime < ntime; itime++) {
        ...
        ss.fill('0');
        ss << fixed << tau_local;
        string localTauFormatted = ss.str();

        // Temperature output files
        string TempFileName= "results/Tcontour"+localTauFormatted;
        std::ofstream outputTemp;
        outputTemp.open(TempFileName);    

        // Velocity output files
        string VelFileName= "results/Vcontour"+localTauFormatted;
        std::ofstream outputVel;
        outputVel.open(VelFileName);    
        ...
        for (int i = 0; i < nx; i++) {
            ...
            for (int j = 0; j < ny; j++) {
                ...
                // Write out Temperature if greater than 0.05 GeV
                double temp_local = fluidCellptr->temperature;
                if (temp_local > 0.05){
                    outputTemp << setw(18) << setprecision(8)
                    << x_local << "   " << y_local << "   " << temp_local << "   " << endl;
                } else {
                    outputTemp << setw(18) << setprecision(8)
                    << x_local << "   " << y_local << "   " << 0.0 << "   " << endl ;
                }
                // Write out velocity
                double vx_local = fluidCellptr->vx;
                double vy_local = fluidCellptr->vy;
                outputVel << setw(18) << setprecision(8)
                << x_local << "   " << y_local << "   " << vx_local << "   " << vy_local << "   " << endl;
            }
        }
        // output << endl;
        outputTemp.close();
